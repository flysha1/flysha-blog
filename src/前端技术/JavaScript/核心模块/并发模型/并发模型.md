...
JavaScript 解释器可以说是 JavaScript 虚拟机，负责 JavaScript 代码的解析和执行。这里 [编译阶段](/blog/前端技术/JavaScript/核心模块/编译阶段/1.html) 有详细解读。
...
>[!warning]
>JavaScript 的单线程是指一个程序进程（在浏览器运行环境中运行的就是浏览器进程）中只有一个 JavaScript 的执行线程，同一时刻内只会有一段 JavaScript 代码在执行。而异步机制是运行环境的两个或以上常驻线程共同完成的。
...
1. 所有同步任务及异步任务按照 [编译原理](/blog/前端技术/JavaScript/核心模块/编译阶段/1.html) 在主线程上执行，形成一个 [执行上下文栈](/blog/前端技术/JavaScript/核心模块/执行阶段/1.html)（Execution Context Stack）
